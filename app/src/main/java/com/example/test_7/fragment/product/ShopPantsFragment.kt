...
import kotlinx.android.synthetic.main.fragment_shop_pants.*

class ShopPantsFragment : Fragment(R.layout.fragment_shop_pants), ShopListListener {
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        setRecyclerViewData()

        iv_fragment_pants_back.setOnClickListener {
            activity?.onBackPressed()
        }
    }

    private fun setRecyclerViewData() {
        rv_fragment_pants_one.layoutManager =
            LinearLayoutManager(requireActivity(), RecyclerView.VERTICAL, false)
        rv_fragment_pants_one.adapter = ShopListAdapterProduct(
            listItems = DummyData().shopListPants, listener = this@ShopPantsFragment
        )

        rv_fragment_pants_two.layoutManager =
            LinearLayoutManager(requireActivity(), RecyclerView.VERTICAL, false)
        rv_fragment_pants_two.adapter = ShopListAdapterProduct(
            listItems = DummyData().shopListPants2, listener = this@ShopPantsFragment
        )
    }
